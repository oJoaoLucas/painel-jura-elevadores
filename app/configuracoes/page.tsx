...
import { falar, prepararVoz } from "@/lib/som";
...
export default function ConfiguracoesPage() {
  ...
  const testarVoz = () => {
    if (!falar("Teste de voz da Jura Auto Center.", config.volume)) {
      avisar("Seu navegador não tem voz disponível.");
    }
  };
      ...
      <main className="px-4 pb-12 sm:px-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-xl bg-jura-card p-6">
            <h2 className="section-title mb-5 text-lg">Som e voz</h2>
            ...
            <div className="space-y-5">
              ...
              <label className="flex cursor-pointer items-center justify-between gap-4">
                <span>
                  ...
                  <span className="text-sm text-jura-muted">
                    Liga/desliga todo o som (beeps e voz)
                  </span>
                </span>
                <input
                  type="checkbox"
              ...
              <label
                className={`flex items-center justify-between gap-4 ${
                  config.som_ativo ? "cursor-pointer" : "opacity-40"
                }`}
              >
                <span>
                  <span className="block font-semibold">Voz falada</span>
                  <span className="text-sm text-jura-muted">
                    Anúncios em voz (ex.: &quot;Elevador 1 pronto&quot;). Desligada,
                    usa só beep.
                  </span>
                </span>
                <input
                  type="checkbox"
                  disabled={!config.som_ativo}
                  checked={config.voz_ativa}
                  onChange={(e) => salvar({ voz_ativa: e.target.checked })}
                  className="h-6 w-6 shrink-0 accent-jura-red"
                />
              </label>
              ...
              <button
                onClick={testarVoz}
                className="rounded-lg border border-jura-border px-4 py-2 font-bold uppercase tracking-wide text-jura-ink transition-colors hover:border-jura-red"
              >
                Testar voz
              </button>
            </div>
