...
**Tabelas:**
- `elevadores` — 4 fixos (id 1–4). Campos: `status` (`livre`|`ocupado`|`aguardando`|`pronto`), `placa`, `carro`, `servico` (uma linha por serviço), `mecanico`, `ocupado_em` (início, p/ cronômetro), `pausado_em` (cronômetro pausado — almoço/fechado; null = correndo), `updated_at`.
...
- **`pausado_em`** (botão Pausar/Retomar na Recepção): pausar grava a hora; a TV mostra o card azul "Pausado" com o tempo congelado e o alerta de demora não dispara. Retomar (ou Liberar ainda pausado) desconta o tempo parado empurrando `ocupado_em`/`entrada` pra frente — almoço/fechado não conta no cronômetro nem no relatório. Liberar/aguardando/ocupar novo sempre limpam a pausa.
